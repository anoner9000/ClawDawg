...
- ACE ingestion entrypoint: `./scripts/ingest_ace.sh` (delegates to `modules/briefings/scripts/ingest_ace.sh`). Default manifests + history live under `~/logs/ingest/` (history JSONL: `~/logs/ingest/history.jsonl`). Use `--dry-run`/`-n` to suppress writes or `--manifest-dir DIR` to override paths. Dry-run mode still emits the manifest + history entries but skips all filesystem writes (no Calendar copies, no link edits).
