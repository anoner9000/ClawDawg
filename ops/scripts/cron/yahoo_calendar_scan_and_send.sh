...
import imaplib
import socket, faulthandler, signal
faulthandler.enable()
faulthandler.register(signal.SIGUSR1)
socket.setdefaulttimeout(30)
print("PY: bootstrap ok", flush=True)
...
# ---------------------------
# IMAP safe fetch helpers (live mode)
# ---------------------------
def imap_set_timeout(M, timeout: int):
    # imaplib uses a socket under the hood; settimeout prevents indefinite hangs
    for attr in ("sock", "socket"):
...
def imap_fetch_payload(M, msg_id: bytes, what: str, *, timeout=30, retries=2) -> bytes:
    imap_set_timeout(M, timeout)
    last = None
    for _ in range(retries + 1):
        try:
            typ, dat = M.fetch(msg_id, what)
            if typ != "OK" or not dat:
                raise RuntimeError(f"fetch {what} failed: {typ}")
            for item in dat:
                if isinstance(item, tuple) and len(item) == 2 and isinstance(item[1], (bytes, bytearray)):
                    return bytes(item[1])
            raise RuntimeError(f"fetch {what} returned no payload")
        except Exception as e:
            last = e
            continue
    raise last

def build_min_rfc822(header_bytes: bytes, body_bytes: bytes) -> bytes:
    # Minimal RFC822 bytes for message_from_bytes
...
def main():
    ...
    if DRY_RUN:
        ...
        MAX_LIVE = 80  # safe default; adjust as you like
        if len(msg_ids) > MAX_LIVE:
            msg_ids = msg_ids[-MAX_LIVE:]
            print(f"IMAP: limiting to newest {MAX_LIVE} messages", flush=True)
        for mid in msg_ids:
            # Log progress so we can identify any problematic msg quickly
            try:
                mid_print = mid.decode(errors="ignore")
            except Exception:
                mid_print = str(mid)
            print(f"IMAP: fetching msg_id={mid_print}", flush=True)

            # Fast path: header + text (avoids huge attachments)
            try:
                hdr = imap_fetch_payload(M, mid, "(BODY.PEEK[HEADER])", timeout=20)
                txt = imap_fetch_payload(M, mid, "(BODY.PEEK[TEXT])", timeout=30)
                raw = build_min_rfc822(hdr, txt)
                msg = message_from_bytes(raw)

                # Fallback: if TEXT is empty or extract_text() returns nothing,
                # fetch full message as last resort (still with timeout + limited retries)
                if (not txt) or (not extract_text(msg)):
                    print(f"IMAP: fallback RFC822 msg_id={mid_print}", flush=True)
                    full = imap_fetch_payload(M, mid, "(RFC822)", timeout=45, retries=1)
                    msg = message_from_bytes(full)

                raw_msgs.append(msg.as_bytes())
            except Exception as e:
                print(f"IMAP: fetch failed msg_id={mid_print}: {e}", flush=True)
                continue
    ...
    for raw in raw_msgs:
        ...
        for e in events:
            ...
            safe_title = re.sub(r'[^A-Za-z0-9 _.-]+', '', title).strip() or "Event"
            filename = f"{mmddyyyy}_{safe_title}.ics"
            out_path = ICSDIR / filename

            if DRY_RUN:
                print(f"- {mmddyyyy} | {title} | {grade} -> {child} | {e['time_note']} | file: {filename}")
                continue
            ...
            if out_path.exists():
                i = 2
                while True:
                    filename = f"{mmddyyyy}_{safe_title}_{i}.ics"
                    out_path = ICSDIR / filename
